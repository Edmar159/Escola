...
$result = mysqli_query($con, "SELECT * FROM professor WHERE codProfessor = '$codProf'");
		...
		if(mysqli_num_rows($result) > 0)
		{
			...
			<div class="col-md-12 col-md-offset-0">
				...
				if($usuario = mysqli_fetch_object($result))
				{
					?>
				<form class="form-horizontal" method="POST" action="updateUsuario.php" >
				<input type="hidden" name="codProf" value="<?php echo $codProf ?>">	
				<div class="panel panel-primary">
					<div class="panel-heading">
						<h3 class="panel-title">Dados pessoais</h3>				
					</div>
					<div class="panel-body">
					    ...
					    <label class="col-md-3	 control-label">Formação</label>
						<div class="col-md-8">
						   	<input type="text" class="form-control" name="formacao" placeholder=<?php echo $usuario->formacao ?>>
						</div>
					</div>
					<div class="form-group">
					    <label class="col-md-3	 control-label">Salário</label>
						<div class="col-md-8">
						   	<input type="text" class="form-control" name="salario" placeholder=<?php echo $usuario->salario ?>>
						</div>
					</div>
					</div>
				</div>
